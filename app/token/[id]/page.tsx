...
export default function TokenTrackPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const channel = supabase
      ...
      .on("broadcast", { event: "token_update" }, ({ payload }) => {
        if (payload.tokenId === tokenId) {
          setStatus((prev) => {
            if (payload.status === "served" && prev !== "served") {
              playSynthSound("smooth-served");
            }
            return payload.status as TokenStatus;
          });
        }
      })
      .subscribe();
  ...
  return (
    <div className="min-h-screen bg-[var(--gold-bg)] max-w-lg mx-auto px-4 py-6">
      ...
      {/* Token display */}
      <div className={`card-warm border-2 border-[var(--amber-brand)] py-8 text-center mb-5 transition-all duration-300 ${
        status === "served" ? "opacity-40 grayscale pointer-events-none" : ""
      }`}>
